...
function PortfolioExperience({ portInfoNumber, portInfoTitle }) {
  ...
  return (
    <div className="flex items-center gap-1 md:gap-4 w flex-col md:flex-row">
      <p className="text-7xl font-bold">{Math.floor(count).toLocaleString()}</p> {/* Formatting the count */}
      <span className="inline-block h-[50px] border-b-[1px] md:border-l-[1px] border-emerald-950" />
      <p>{portInfoTitle}</p>
    </div>
  );
}
